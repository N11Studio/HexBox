...
board.on("ready", function() {
  ...
  five.Pin.read(sensor1_right, function(error, rightsensor) {
    ...
    if (pinval_right !== 0) {
      rightArray = Array.from({length:5},()=> (rightsensor))
      var total_right = 0;
  ...
  five.Pin.read(sensor2_front, function(error, frontsensor) {
    ...
    if (pinval_front !== 0) {
      frontArray = Array.from({length:5},()=> (frontsensor))
      var total_front = 0;
  ...
  five.Pin.read(sensor3_left, function(error, leftsensor) {
    ...
    if (pinval_left !== 0) {
      leftArray = Array.from({length:5},()=> (leftsensor))
      var total_left = 0;
  ...
  five.Pin.read(sensor4_top, function(error, topsensor) {
    ...
    if (pinval_top !== 0) {
      topArray = Array.from({length:5},()=> (topsensor))
      var total_top = 0;
  ...
  five.Pin.read(sensor5_bottom, function(error, bottomsensor) {
    ...
    if (pinval_bottom !== 0) {
      bottomArray = Array.from({length:5},()=> (bottomsensor))
      var total_bottom = 0;
  ...
  five.Pin.read(sensor6_back, function(error, backsensor) {
    ...
    if (pinval_back !== 0) {
      backArray = Array.from({length:5},()=> (backsensor))
      var total_back = 0;
